...
func (m *Manager) toRepositoryCampaign(campaign *models.Campaign) *repository.Campaign {
    ...
    // Fix 1: State lives on CampaignInstance, not models.Campaign.
    // Pass "idle" as default — the DB column default is also 'idle'.
    state := "idle"

    // Fix 2: CampaignPriority is a string type, map it to int for the DB column.
    ...
    // Fix 3: CampaignRuntimeConfig has no ToMap() — marshal via JSON instead.
    ...
    repoCamp := &repository.Campaign{
        ...
        Status:          status,
        State:           state,          // Fix 1: hardcoded "idle" for new campaigns
        Priority:        priority,       // Fix 2: int conversion via map
        ScheduledAt:     campaign.Schedule.ScheduledStartTime,
        TotalRecipients: int(campaign.TotalRecipients),
        Config:          configMap,      // Fix 3: JSON round-trip to map
        TemplateIDs:     campaign.TemplateIDs,
        AccountIDs:      campaign.AccountIDs,
        RecipientListID: nil,            // uuid nullable — nil = NULL in DB
        ProxyIDs:        []string{},
        Tags:            []string{},
    ...
    fmt.Printf("DEBUG toRepositoryCampaign: id=%s state=%s status=%s priority=%d templateIDs=%v accountIDs=%v\n",
        repoCamp.ID, repoCamp.State, repoCamp.Status, repoCamp.Priority,
        repoCamp.TemplateIDs, repoCamp.AccountIDs)
