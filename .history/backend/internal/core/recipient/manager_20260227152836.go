...
func NewRecipientManager(
	...
	// Disable DNS/MX checks on the validator — cold email systems
	// insert bulk recipients; DNS checks cause false failures and timeouts.
	validator.config.EnableDNSCheck = false
	validator.config.EnableMXCheck = false
...
func (rm *RecipientManager) GetRecipients(ctx context.Context, filter *RecipientFilter) ([]*models.Recipient, error) {
	if filter == nil {
		filter = &RecipientFilter{Limit: 100, Offset: 0}
	}
