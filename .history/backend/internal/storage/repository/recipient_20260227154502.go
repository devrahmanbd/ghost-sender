...
func (r *RecipientRepository) List(ctx context.Context, filter *RecipientFilter) ([]*Recipient, int, error) {
	...
	// Count query
	countQuery := fmt.Sprintf("SELECT COUNT(*) FROM recipients %s", whereSQL)
	var total int
	if err := r.db.QueryRowContext(ctx, countQuery, args...).Scan(&total); err != nil {
		fmt.Printf("🔴 DEBUG repo.List COUNT error: %v\n", err)
		return nil, 0, fmt.Errorf("failed to count recipients: %w", err)
	}
	...
	if filter != nil && filter.Limit > 0 {
		limitSQL = fmt.Sprintf(" ORDER BY %s %s LIMIT $%d OFFSET $%d",
		...
		limitSQL = fmt.Sprintf(" ORDER BY %s %s LIMIT 100 OFFSET 0", sortBy, sortOrder)
	}
	...
	query := fmt.Sprintf(`
		...
		FROM recipients %s%s`, whereSQL, limitSQL)

	fmt.Printf("🟢 DEBUG repo.List query: %s args: %v\n", query, args)

	rows, err := r.db.QueryContext(ctx, query, args...)
	if err != nil {
		fmt.Printf("🔴 DEBUG repo.List QUERY error: %v\n", err)
		return nil, 0, fmt.Errorf("failed to list recipients: %w", err)
	}
	defer rows.Close()
