...
func (a *recipientManagerAdapter) Create(ctx context.Context, req *CreateRecipientReq) (*models.Recipient, error) {
	...
	if req.CampaignID != nil {
		rec.CampaignID = *req.CampaignID
	}
	if err := a.core.AddRecipient(ctx, rec); err != nil {
		return nil, err
	}
    if req.CampaignID != nil {  // <- Nil panic here?
        rec.CampaignID = *req.CampaignID
    }
    fmt.Printf("🟢 DEBUG calling core.AddRecipient(rec=%+v)\n", rec)
    if err := a.core.AddRecipient(ctx, rec); err != nil {
        fmt.Printf("🔴 DEBUG AddRecipient ERROR: %v\n", err)
        return nil, err
    }
    return rec, nil
}

// List — opts.CampaignID is *string; GetRecipients/CountRecipients take *RecipientFilter.
...
func (a *recipientManagerAdapter) GetStats(ctx context.Context, campaignID *string) (interface{}, error) {
    ...
    return a.core.GetStatistics(ctx, cid)
}
