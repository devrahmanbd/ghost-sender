...
func initCoreServices(ctx context.Context, repos *repositories, cfg *config.AppConfig, log logger.Logger, wsHub *websocket.Hub) (*CoreServices, error) {
	...
	// FIX: Pass ctx as first argument — this is the compile error.
	// Previously only 3 args were passed; NewAccountManager now requires 4.
	...
	recipientValidator := recipient.NewValidator()
	...
	personalizationMgr := personalization.NewManager(
		log,
