...
func (a *senderTemplateAdapter) GetNextTemplate(ctx context.Context) (models.Template, error) {
	...
	if err != nil || len(templates) == 0 {
		return models.Template{}, fmt.Errorf("no templates available")
	}
	t := templates[0]
...
// ─────────────────────────────────────────────────────────
// sender.DeliverabilityManager adapter (simple pass-through)
// ─────────────────────────────────────────────────────────
...
func (s *smtpEmailProvider) Send(ctx context.Context, msg sender.EmailMessage) (string, error) {
	...
	messageID, err := s.provider.SendEmail(ctx, email)
	if err != nil {
		return "", err
	}
...
// ─────────────────────────────────────────────────────────
// sender.RateLimiter — no-op for now (can add later)
// ─────────────────────────────────────────────────────────
...
// adapt template.TemplateManager (pointer) for handlers — already exists, this is for the sender engine
