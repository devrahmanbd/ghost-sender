...
func (p *SMTPConnectionPool) dialSMTP() (*smtp.Client, error) {
        ...
        if p.smtpConfig.Port != 465 {
                tlsConfig := &tls.Config{
                        ServerName:         p.smtpConfig.Host,
                        InsecureSkipVerify: p.smtpConfig.TLSConfig != nil && p.smtpConfig.TLSConfig.InsecureSkipVerify,
                }
                ...
                } else if p.smtpConfig.TLSConfig != nil && p.smtpConfig.TLSConfig.Enabled {
                        // TLS is required but server doesn't support STARTTLS
                        conn.Close()
                        return nil, fmt.Errorf("server does not support STARTTLS on port %d", p.smtpConfig.Port)
                }
        }

        // Authenticate only AFTER TLS is established
        if p.smtpConfig.Password != "" {
