...
// toRepoRecipient — add Name field so the DB name column round-trips correctly
func toRepoRecipient(m *models.Recipient) *repository.Recipient {
    ...
    return &repository.Recipient{
        ...
        ListID:    m.CampaignID,
        Email:     m.Email,
        Name:      m.FullName,  // ← maps to DB `name` column
...
// toModelRecipient — derive FullName from available name fields
func toModelRecipient(r *repository.Recipient) *models.Recipient {
    ...
    // Derive FullName — priority: FirstName+LastName → either alone → DB name column
    switch {
    ...
    case r.Name != "":
        // Fallback for old rows where only the `name` column was stored
        m.FullName = r.Name
    }
...
func toRepoFilter(f *RecipientFilter) *repository.RecipientFilter {
    if f == nil {
        return &repository.RecipientFilter{Limit: 100}
    }

    repoFilter := &repository.RecipientFilter{
        Limit:     f.Limit,
        Offset:    f.Offset,
        SortBy:    f.SortBy,
    ...
    // ✅ Only set non-empty values — never pass "" as UUID or blank status filter
    if f.CampaignID != "" {
        repoFilter.ListIDs = []string{f.CampaignID}
    }
    if string(f.Status) != "" {
        repoFilter.Status = []string{string(f.Status)}
    }
    if f.Email != "" {
        repoFilter.Emails = []string{f.Email}
    }
    if f.SearchTerm != "" {
        repoFilter.Search = f.SearchTerm
    }
