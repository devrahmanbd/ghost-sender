...
// isValidUUID returns false for empty strings and non-UUID values
// prevents pq: invalid input syntax for type uuid: "" errors
...
func (h *RecipientHandler) ListRecipients(w http.ResponseWriter, r *http.Request) {
    ...
    var campID *string
    if campaignID != "" {
        // ✅ Extra guard: only pass if it's a valid UUID
        if !isValidUUID(campaignID) {
            h.respondError(w, errors.BadRequest("Invalid campaign_id format"))
            return
        }
        campID = &campaignID
    }
...
func (h *RecipientHandler) DeleteFirst(w http.ResponseWriter, r *http.Request) {
    ...
    // ✅ Validate UUID before passing to repo
    if req.CampaignID != "" && !isValidUUID(req.CampaignID) {
        h.respondError(w, errors.BadRequest("Invalid campaign_id format"))
        return
    }
    ...
    h.respondJSON(w, http.StatusOK, map[string]interface{}{
        "message": "First recipients deleted successfully",
        "count":   deleted,
    })
}
...
func (h *RecipientHandler) DeleteLast(w http.ResponseWriter, r *http.Request) {
    ...
    h.respondJSON(w, http.StatusOK, map[string]interface{}{
        "message": "Last recipients deleted successfully",
        "count":   deleted,
    })
}
...
func (h *RecipientHandler) respondError(w http.ResponseWriter, err error) {
    ...
    default:
        errMsg := err.Error()
        h.logger.Error("recipient handler internal error",
            logger.Field{Key: "error", Value: errMsg})

        // ✅ Detect pq UUID errors and return 400 instead of leaking 500
        if strings.Contains(errMsg, "invalid input syntax for type uuid") {
            status = http.StatusBadRequest
            message = "Invalid UUID in request parameters"
        } else if strings.Contains(errMsg, "duplicate key") || strings.Contains(errMsg, "unique_violation") {
