...
func (r *AccountRepository) List(ctx context.Context, filter *AccountFilter) ([]*Account, int, error) {
        ...
        allowedAccountCols := []string{"id", "name", "email", "provider", "status", "health_score", "sent_today", "total_sent", "success_rate", "created_at", "updated_at", "last_used_at", "daily_limit"}
        sortBy := "created_at"
        sortOrder := "DESC"
        if filter != nil {
                sortBy = sanitizeSortColumn(filter.SortBy, "created_at", allowedAccountCols)
