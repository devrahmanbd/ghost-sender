...
func (r *CampaignRepository) Update(ctx context.Context, campaign *Campaign) error {
        ...
        checkpointJSON, err := json.Marshal(campaign.CheckpointData)
        if err != nil {
...
func (r *CampaignRepository) List(ctx context.Context, filter *CampaignFilter) ([]*Campaign, int, error) {
        ...
        allowedCampaignCols := []string{"id", "name", "status", "state", "priority", "created_at", "updated_at", "scheduled_at", "started_at", "completed_at", "total_recipients", "sent_count", "failed_count", "success_rate", "progress"}
        sortBy := sanitizeSortColumn(filter.SortBy, "created_at", allowedCampaignCols)
...
func (r *CampaignRepository) UpdateProgress(ctx context.Context, id string, sent, failed, pending int) error {
        ...
        query := `
                UPDATE campaigns SET
                        ...
                        failed_count = $3,
                        pending_count = $4,
                        total_recipients = GREATEST(total_recipients, $5::integer),
                        progress = CASE WHEN GREATEST(total_recipients, $5::integer) > 0 THEN (($2::integer + $3::integer) * 100.0 / GREATEST(total_recipients, $5::integer)) ELSE 0 END,
                        success_rate = CASE WHEN ($2::integer + $3::integer) > 0 THEN ($2::integer * 100.0 / ($2::integer + $3::integer)) ELSE 0 END,
                        updated_at = $6
                WHERE id = $1`

        _, err := r.db.ExecContext(ctx, query, id, sent, failed, pending, total, time.Now())
        return err
}
