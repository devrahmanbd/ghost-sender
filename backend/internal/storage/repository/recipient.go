...
func (r *RecipientRepository) Count(ctx context.Context, filter *RecipientFilter) (int, error) {
    ...
    if filter != nil {
        ...
        if len(filter.ListIDs) > 0 {
            whereClauses = append(whereClauses, fmt.Sprintf("recipient_list_id = ANY($%d)", argPos))
...
func (r *RecipientRepository) CountByList(ctx context.Context, listID string) (int, error) {
    query := `SELECT COUNT(*) FROM recipients WHERE recipient_list_id = $1`
...
func (r *RecipientRepository) CountByListAndStatus(ctx context.Context, listID, status string) (int, error) {
    query := `SELECT COUNT(*) FROM recipients WHERE recipient_list_id = $1 AND status = $2`
...
func (r *RecipientRepository) GetMostEngaged(ctx context.Context, listID string, limit int) ([]*Recipient, error) {
    ...
    if listID != "" {
        query = `
            SELECT id, recipient_list_id, email, name, engagement_score, opens, clicks, 
                   sent_count, last_open_at, last_click_at, created_at, updated_at
            FROM recipients
            WHERE recipient_list_id = $1 AND sent_count > 0
...
func (r *RecipientRepository) GetLeastEngaged(ctx context.Context, listID string, limit int) ([]*Recipient, error) {
    ...
    if listID != "" {
        query = `
            ...
            FROM recipients
            WHERE recipient_list_id = $1 AND sent_count > 0 AND unsubscribed = false
...
func (r *RecipientRepository) GetRecentlyAdded(ctx context.Context, listID string, limit int) ([]*Recipient, error) {
    ...
    if listID != "" {
        query = `
            SELECT id, recipient_list_id, email, name, status, is_valid, created_at, updated_at
            FROM recipients
            WHERE recipient_list_id = $1
...
func (r *RecipientRepository) GetInactiveRecipients(ctx context.Context, listID string, inactiveDays int, limit int) ([]*Recipient, error) {
    ...
    if listID != "" {
        query = `
            SELECT id, recipient_list_id, email, name, engagement_score, last_open_at, 
    ...
    rec.Status = "active"
        ...
        query := `
                INSERT INTO recipients
                        (recipient_list_id, email, name, first_name, last_name, status, custom_fields, metadata, created_at, updated_at)
                VALUES
...
func (r *RecipientRepository) GetByID(ctx context.Context, id string) (*Recipient, error) {
        query := `
                SELECT id, recipient_list_id, email,
...
func (r *RecipientRepository) List(ctx context.Context, filter *RecipientFilter) ([]*Recipient, int, error) {
        ...
        allowedRecipientCols := []string{"id", "recipient_list_id", "email", "name", "first_name", "last_name", "status", "created_at", "updated_at"}
        sortBy := "created_at"
        sortOrder := "DESC"
        if filter != nil {
                sortBy = sanitizeSortColumn(filter.SortBy, "created_at", allowedRecipientCols)
...
func (r *RecipientRepository) BulkInsert(ctx context.Context, recs []*Recipient) error {
        ...
        baseQuery := `
                INSERT INTO recipients (
                        id, recipient_list_id, email, name, first_name, last_name, domain, status, source,
...
func (r *RecipientRepository) BulkDeleteByList(ctx context.Context, listID string) error {
        query := `DELETE FROM recipients WHERE recipient_list_id = $1`
...
func (r *RecipientRepository) GetStats(ctx context.Context, listID string) (*RecipientStats, error) {
        ...
        scanDest := []interface{}{
                &stats.TotalRecipients, &stats.ValidRecipients, &stats.InvalidRecipients,
                &stats.Subscribed, &stats.Unsubscribed, &stats.HardBounced,
                &stats.SoftBounced, &stats.WithComplaints, &stats.TotalSent,
                &stats.TotalFailed, &stats.AverageEngagement,
        }

        var err error
        if listID != "" {
                err = r.db.QueryRowContext(ctx, baseSelect+" WHERE recipient_list_id = $1", listID).Scan(scanDest...)
        } else {
                err = r.db.QueryRowContext(ctx, baseSelect).Scan(scanDest...)
        }
        if err != nil {
                return nil, err
        }
        ...
        if listID != "" {
                domainRows, derr := r.db.QueryContext(ctx, `SELECT SPLIT_PART(email, '@', 2) as domain, COUNT(*) FROM recipients WHERE recipient_list_id = $1 GROUP BY domain`, listID)
                if derr == nil {
                ...
                statusRows, serr := r.db.QueryContext(ctx, `SELECT status, COUNT(*) FROM recipients WHERE recipient_list_id = $1 GROUP BY status`, listID)
                if serr == nil {
                ...
                domainRows, derr := r.db.QueryContext(ctx, `SELECT SPLIT_PART(email, '@', 2) as domain, COUNT(*) FROM recipients GROUP BY domain`)
                if derr == nil {
                        defer domainRows.Close()
                        for domainRows.Next() {
                                var domain string
                ...
                statusRows, serr := r.db.QueryContext(ctx, `SELECT status, COUNT(*) FROM recipients GROUP BY status`)
                if serr == nil {
                        defer statusRows.Close()
                        for statusRows.Next() {
                                var status string
